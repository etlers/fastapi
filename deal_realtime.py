...
class Kiwoom(QAxWidget):
    ...
    def kiwoom_TR_OPT10003(self, strCode, **kwargs):
        ...
        """
        res = self.kiwoom_SetInputValue("종목코드", strCode)
        res = self.kiwoom_CommRqData("체결정보", "OPT10003", 0, 화면번호)
        return res
    ...
    @SyncRequestDecorator.kiwoom_sync_request
    def kiwoom_TR_OPT10085(self, 계좌번호, **kwargs):
        """계좌수익률요청
        :param 계좌번호: 계좌번호
        :param kwargs:
        :return:
    ...
    @SyncRequestDecorator.kiwoom_sync_request
    def kiwoom_TR_OPW00001(self, 계좌번호, **kwargs):
        """예수금상세현황요청
        :param 계좌번호: 계좌번호
        :param kwargs:
        :return:
...
def execute_logic(hts):
    ...
    order_div = 1
    # 거래내역 저장
    f = open("./deal/" + file_name, 'a', encoding="utf-8-sig")
    # 순번
    idx = 0
    while True:
        ...
        if order_div == 1:
            ...
            if now_price == std_price:
                ...
                order_cnt = 1
                ###############################################################
                f.write("# Buy - " + str(std_price) + "\t" + str(now_price) + "\t" + str(order_cnt))
                hts.kiwoom_SendOrder("계좌수익률요청", "1234", "8162124111", order_div, "122630", order_cnt, 0, "03", "")
                order_div = 2
                std_price = now_price - gap_amount
            # 현재가격이 기준 가격보다 적으면 기준 가격을 변경
            elif now_price + gap_amount < std_price:
                std_price = now_price + gap_amount
            print(idx, "]", std_price, now_price)
        # 매도인 경우 현재가격이 기준 가격보다 크면 기준 가격을 변경
        else:
            # 기준가격과 현재 가격이 같으면 매도
            if now_price == std_price:
                # 수량 계산. 증액해 계산
                f.write("# Sell - " + str(std_price) + "\t" + str(now_price) + "\t" + str(order_cnt))
                hts.kiwoom_SendOrder("계좌수익률요청", "1234", "8162124111", order_div, "122630", order_cnt, 0, "03", "")
                order_cnt = 0
                order_div = 1
                std_price = now_price + gap_amount
    ...
    # 마지막에 수량이 남아있으면 털고 나감
    if order_cnt > 0:
        #print("ending sell", std_price, now_price, order_cnt)  
        f.write("# ending sell - " + str(std_price) + "\t" + str(now_price) + "\t" + str(order_cnt))  
        hts.kiwoom_SendOrder("계좌수익률요청", "1234", "8162124111", 2, "122630", 1, 0, "03", "")
